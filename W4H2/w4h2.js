...
class UserManager{
    constructor() {
        ...
        this.sessionKey = 'users_restored_this_session';
        this.container = null;
        this.users = [];
        this.observer = null;
    ...
    createContainer() {
        let existingContainer = document.querySelector('.ins-api-users');
        ...
        this.container.className = 'ins-api-users';
        this.container.id = 'user-manager-app';
        ...
        if (targetElement) {
            ...
            console.log(`✅ Container "${appendLocation}" selector'ına eklendi`);
        } else {
            console.error(`❌ Selector "${appendLocation}" bulunamadı! Body'ye ekleniyor...`);
            document.body.appendChild(this.container);
            console.log('✅ Container body\'ye eklendi');
        }
    }
  ...
  deleteUser(userId) {
    ...
    async refreshData() {
        try {
            ...
            this.showError('Veriler yenilenirken hata: ' + error.message);
        }
    }
    ...
    setupMutationObserver() {
        this.observer = new MutationObserver((mutationsList, observer) => {
            ...
            if (userCards.length === 0 && this.users.length === 0) {
                this.showRestoreButton();
            } else {
                const existingRestore = this.container.querySelector('.restore-section');
                if (existingRestore) {
    ...
    showRestoreButton() {
        const alreadyUsed = sessionStorage.getItem(this.sessionKey);
        if (alreadyUsed) {
            return;
        }

        const restoreButton = document.createElement('div');
        restoreButton.className = 'restore-section';
        restoreButton.innerHTML = `
            <div class="restore-message">
                <h3>📭 Tüm kullanıcılar silindi!</h3>
                <p>Kullanıcıları geri yüklemek için aşağıdaki butona tıklayın.</p>
                <button class="restore-btn" onclick="userManager.restoreUsers()">
                    🔄 Kullanıcıları Geri Yükle
                </button>
            </div>
        `;

        this.container.appendChild(restoreButton);
    }
    ...
    async restoreUsers() {
        try {
            ...
            const restoreSection = this.container.querySelector('.restore-section');
            if (restoreSection) {
            ...
            this.showNotification('✅ Kullanıcılar başarıyla geri yüklendi!', 'success');
        } catch (error) {
            this.showError('Kullanıcılar geri yüklenirken hata: ' + error.message);
        }
    }
